...
class Info {
  public:
     ...
     *   In your subclass you need to implement this function.
     */
    static Info* join(Info * info1, Info * info2);
};
...
class DataFlowAnalysis {
  ...
  private:
		...
		void addEdge(Instruction * src, Instruction * dst, Info * content) {
			...
			if (EdgeToInfo.count(edge) == 0)
				EdgeToInfo[edge] = new Info(*content);
			return;
		}
		...
		void initializeBackwardMap(Function * func) {
			...
			for (Function::iterator bi = func->begin(), e = func->end(); bi != e; ++bi) {
				...
				// Initialize outgoing edges from the first instruction of the current basic block to the 
				// last instructions of it's predecessors
				for (auto pi = pred_begin(block), pe = pred_end(block); pi != pe; ++pi) {
					BasicBlock * prev = *pi;					
				...
				// If there is at least one phi node, add an edge to the first phi node
				// from the first non-phi node instruction in the basic block.
				if (isa<PHINode>(firstInstr)) {
				...
				// Initialize incoming edges to the last instruction of current basic block
				// From the first instructions of it's successor basic blocks
				Instruction * term = (Instruction *)block->getTerminator();
				for (auto si = succ_begin(block), se = succ_end(block); si != se; ++si) {
					BasicBlock * succ = *si;
					Instruction * next = &(succ->front());
			...
			//In case of the forward analysis we had one entry node which was a null pointer with index,
			//and which had an outgoing edge to the first instruction in the function
			//but in the backward case we will have an exit instructions which will be null pointer and 
			//that will have outgoing edges to all the return instructions in the function
			// EntryInstr = (Instruction *) &((func->front()).front());
			// addEdge(nullptr, EntryInstr, &InitialState);
		...
		void initializeForwardWorklist(Function * func, std::deque<unsigned> * worklist){
			for (inst_iterator I = inst_begin(func), E = inst_end(func); I != E; ++I){
				Instruction * instr = &*I;
				worklist->push_back(InstrToIndex[instr]);  
			}
		}

		void initializeBackwardWorklist(Function * func, std::deque<unsigned> * worklist){
			for (inst_iterator I = inst_begin(func), E = inst_end(func); I != E; ++I){
				Instruction * instr = &*I;
				//fill the worklist in the reverse order so that the iterative algorithm runs faster
				//you can also try to reverse this and see that you are still getting the same answer 
				//in case of programs without loops.
				worklist->push_front(InstrToIndex[instr]);  
			}
		}
  ...
  public:
    ...
    void runWorklistAlgorithm(Function * func) {
		...
		if (Direction)
    		initializeForwardWorklist(func, &worklist);
    	else
    		initializeBackwardWorklist(func, &worklist);
		

    	// (3) Compute until the work list is empty
		while (!worklist.empty()){
			unsigned inst_index = worklist.front();
			worklist.pop_front();
			Instruction* I = IndexToInstr[inst_index];

			std::vector<unsigned> IncomingEdges;
			std::vector<unsigned> OutgoingEdges;
			std::vector<Info *> OutgoingInfos;
			getIncomingEdges(inst_index, &IncomingEdges);
			getOutgoingEdges(inst_index, &OutgoingEdges);

			flowfunction(I, IncomingEdges, OutgoingEdges, OutgoingInfos);
			for(int i = 0; i < OutgoingInfos.size(); i++){
				Info* result = Info::join(EdgeToInfo[Edge(inst_index, OutgoingEdges[i])], OutgoingInfos[i]);
				if(!Info::equals(EdgeToInfo[Edge(inst_index, OutgoingEdges[i])], result)){
					delete EdgeToInfo[Edge(inst_index, OutgoingEdges[i])];
					EdgeToInfo[Edge(inst_index, OutgoingEdges[i])] = result;
